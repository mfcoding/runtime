...
if(des!=NULL){ strcpy(des+strlen(des), src); }else{____MALLOC_Err_mgs; return EXIT__FAILURE;}
...
if(strlen(des)+strlen(src) < MAX_MALLOC){ strcpy(des+strlen(des), src);} else { \
if(des!=NULL) { puts("Not enough allocated memory"); return EXIT__FAILURE;}} 

#define replace_string(des, src) \
des=(string)realloc((string)des, MAX_MALLOC=strlen(src)+1); \
if(des!=NULL){ strcpy(des, src); }else{____MALLOC_Err_mgs; return EXIT__FAILURE;}
...
src = (string)realloc((string)src, MAX_MALLOC=____c+1); if(src==NULL){____MALLOC_Err_mgs; return EXIT__FAILURE;}
...
delete(ptr); ptr=NULL;}else{____MALLOC_Err_mgs; return EXIT__FAILURE;}}else{____MALLOC_Err_mgs; return EXIT__FAILURE;}

#define get_string(des) \
____c = 0; while(1){ des[____c] = getchar(); \
if(des[____c] == '\n'){des[____c]='\0'; break;} \
else{____c++;des=realloc((string)des, MAX_MALLOC=____c+1);if(des==NULL){____MALLOC_Err_mgs; return EXIT__FAILURE;}}}
